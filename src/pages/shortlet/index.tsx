...
const ShortletPage: FC<IProps> = ({
  ...
  const getMoreProperties = async () => {
    ...
    const res = await axios.get(
      `${process.env.NEXT_PUBLIC_REST_API}/properties?category.name=Shortlet&_start=${isProperties.length}&_limit=9&_sort=${selectedSort}`
    );
...
export async function getServerSideProps() {
  ...
  const lowestPrice = await axios.get(
    `${process.env.NEXT_PUBLIC_REST_API}/properties?category.name=Shortlet&_limit=9&_sort=price:asc`
  );
  const highestPrice = await axios.get(
    `${process.env.NEXT_PUBLIC_REST_API}/properties?category.name=Shortlet&_limit=9&_sort=price:desc`
  );
